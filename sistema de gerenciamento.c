...
bool negativado(CLIENTE* c1) {
	...
	int contaC, divida;
	contaC = c1->valorContaCorrente;
	divida = c1->valorDaDivida;
...
bool obterEmprestimo(CLIENTE* c1, int valor) {
	...
	if (valor > 0){
		if (valor > dividaMaxima && tipo == 'C'){
			return false;
		} else if (valor > dividaMaximaEspecial && tipo == 'E'){
			return false;
		} else {
...
int main(){
	...
	if (obterEmprestimo(&c4, 40000))  printf("Cliente '%s' obteve o emprestimo solicitado.\n", c4.pessoa.nome);
	...
	if (obterEmprestimo(&c2,60000))  printf("Cliente '%s' obteve o emprestimo solicitado.\n", c2.pessoa.nome);
	else  printf("Cliente '%s' nao obteve o emprestimo solicitado.\n", c2.pessoa.nome);
